...
const PLACEMENT = [
  ...
  { section:"Reading", q:"'Okonkwo worked hard to overcome his father's failures.' — Why did he work hard?", opts:["To become rich","To travel","To overcome his father's failures","To win a prize"], ans:2 },
...
function PlacementTest({ onDone }) {
  ...
  const next = () => {
    if (i < PLACEMENT.length - 1) { setI(p=>p+1); setSel(null); setConfirmed(false); }
    else {
      const finalScores = { ...scores };
      if (sel === q.ans) finalScores[q.section]++;
      const total = finalScores.Grammar + finalScores.Vocabulary + finalScores.Reading;
      const level = total >= 11 ? "Advanced" : total >= 6 ? "Intermediate" : "Beginner";
    ...
    <div style={{ minHeight:"100vh", background:"#f0f7f4", display:"flex", flexDirection:"column", alignItems:"center", padding:20, fontFamily:"'Segoe UI',sans-serif" }}>
      <div style={{ width:"100%", maxWidth:440 }}>
        ...
        {q.opts.map((o,oi) => {
          ...
          return (
            <button key={oi} onClick={()=>!confirmed&&setSel(oi)}
              style={{ display:"block", width:"100%", background:bg, border:`2px solid ${border}`, borderRadius:12,
                padding:"12px 16px", marginBottom:10, cursor:confirmed?"default":"pointer",
...
async function saveProgress(userId, module, xpEarned) {
  await supabase.from("daily_progress").upsert({
    user_id: userId, date: todayStr(), module, completed: true, xp_earned: xpEarned
  }, { onConflict: "user_id,date,module" });
  await supabase.from("users").update({ xp: supabase.rpc ? undefined : undefined }).eq("id", userId);
  const { data } = await supabase.from("users").select("xp").eq("id", userId).single();
  if (data) {
    await supabase.from("users").update({ xp: (data.xp || 0) + xpEarned }).eq("id", userId);
  }
}

async function checkTodayProgress(userId) {
  const { data } = await supabase.from("daily_progress")
    .select("module").eq("user_id", userId).eq("date", todayStr()).eq("completed", true);
  return data ? data.map(d => d.module) : [];
}

async function updateStreak(userId) {
  const { data } = await supabase.from("users").select("last_login, streak").eq("id", userId).single();
  if (!data) return;
  ...
  await supabase.from("users").update({ last_login: todayStr(), streak: newStreak }).eq("id", userId);
  return newStreak;
}
      ...
      if (mode === "register") {
        ...
        if (data.user) {
          await supabase.from("users").insert({
            ...
            xp: 0, streak: 1, level: "Beginner", placement_done: false, last_login: todayStr()
          });
          onDone({ id: data.user.id, name: f.name, email: f.email, xp: 0, streak: 1, level: "Beginner", isNew: true });
        }
      } else {
        const { data, error } = await supabase.auth.signInWithPassword({ email: f.email, password: f.pw });
        ...
        if (data.user) {
          ...
          onDone({ ...profile, streak: newStreak || profile?.streak || 1, isNew: !profile?.placement_done });
        }
      }
...
export default function WriteUpApp() {
  ...
  const loadBadges = async (uid) => {
    const { data } = await supabase.from("user_badges").select("badge_name").eq("user_id", uid);
    setBadges(data ? data.map(d => d.badge_name) : []);
  };

  const awardBadge = async (badgeName) => {
    if (badges.includes(badgeName)) return;
    await supabase.from("user_badges").insert({ user_id: user.id, badge_name: badgeName });
  ...
  const afterPlacement = async (result) => {
    setPlacement(result);
    if (user?.id) {
      await supabase.from("users").update({ level: result.level, placement_done: true }).eq("id", user.id);
    }
    setScreen("result");
  };
  ...
  if (screen==="result")  return <LevelResult result={placement} onContinue={()=>setScreen("app")} />;
  ...
  return (
    <div style={{maxWidth:440,margin:"0 auto",minHeight:"100vh",background:"#f0f7f4",fontFamily:"'Segoe UI',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:70}}>
        {activeMod
          ? <ModShell mod={activeMod} level={level} addXp={addXp} onBack={()=>{ setActiveMod(null); loadTodayProgress(user?.id); }} />
          : tab==="home"    ? <Home setMod={setActiveMod} xp={xp} lvl={lvl} pct={pct} level={level} doneToday={doneToday} />
          : tab==="profile" ? <Profile user={user} xp={xp} lvl={lvl} pct={pct} level={level} badges={badges} streak={streak} />
          : tab==="board"   ? <Board userId={user?.id} myXp={xp} />
          : <Settings user={user} onLogout={async()=>{ await supabase.auth.signOut(); setScreen("landing"); setUser(null); }} />
        }
      </div>
...
function ModShell({ mod, level, addXp, onBack }) {
  const earnAndBack = async (xp) => { await addXp(xp, mod.id); };
  return (
    <div style={{padding:18}}>
      ...
      {mod.id==="grammar"    && <DailyGrammar    level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
      {mod.id==="vocabulary" && <DailyVocab      level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
      {mod.id==="peel"       && <DailyPeel       level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
      {mod.id==="reading"    && <DailyReading    level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
      {mod.id==="mistakes"   && <DailyMistakes   level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
      {mod.id==="quiz"       && <DailyQuiz       level={level} addXp={n=>earnAndBack(n)} onBack={onBack} />}
    </div>
  );
}
...
function DoneScreen({ xp, onBack }) {
  return (
    <div style={{textAlign:"center",padding:48}}>
      ...
      <p style={{color:"#888",fontSize:13}}>Progress saved to your account ✅</p>
...
function DailyReading({ level, addXp, onBack }) {
  ...
  if (phase==="read") return (
    <div>
      <Card style={{marginBottom:14}}><div style={{fontSize:11,color:"#888",marginBottom:4}}>📅 Today's Text · {c.topic}</div><h3 style={{color:G,margin:"0 0 12px"}}>{c.title}</h3><p style={{lineHeight:1.9,fontSize:14,color:"#333"}}>{c.passage}</p></Card>
...
function Board({ userId, myXp }) {
  ...
  useEffect(()=>{
    supabase.from("users").select("id,name,xp").order("xp",{ascending:false}).limit(10).then(({data})=>{
      if(data) setLb(data);
    });
  },[]);
  return (
