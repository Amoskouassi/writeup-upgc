    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          max_tokens:1000,
          system:`You are a strict, helpful English writing examiner for ${acadLvl} university students in Côte d'Ivoire. Evaluate PEEL paragraphs fairly. Respond ONLY with valid JSON, no markdown.`,
          messages:[{role:"user",content:`Score this PEEL paragraph. Topic: "${c.prompt}"\n\nPOINT: ${vals.point}\nEXPLANATION: ${vals.explanation}\nEVIDENCE: ${vals.evidence}\nLINK: ${vals.link}\n\nReturn JSON: {point_score:0-25, explanation_score:0-25, evidence_score:0-25, link_score:0-25, total:0-100, passed:bool(total>=60), point_feedback:string, explanation_feedback:string, evidence_feedback:string, link_feedback:string, top_tip:string}`}]
        })
      });
      const data = await res.json();
      const text = data.content?.find(b=>b.type==="text")?.text||"";
      const clean = text.replace(/```json|```/g,"").trim();
      setFb(JSON.parse(clean));
      setAttempts(a=>a+1);
      setPhase("feedback");
    } catch {
      setFb({point_score:0,explanation_score:0,evidence_score:0,link_score:0,total:0,passed:false,point_feedback:"Could not load AI feedback.",explanation_feedback:"",evidence_feedback:"",link_feedback:"",top_tip:"Please check your internet connection and try again."});
      setPhase("feedback");
    }
    setLoading(false);
  };
