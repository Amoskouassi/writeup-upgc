...
const toRTF = rows => {
  ...
  rows.forEach((r,i) => {
    b += `{\\pard\\b\\cf1\\fs26 ${i+1}. ${esc(r.student_code)}\\b0\\cf0\\par}\n`;
    b += `{\\pard\\fs20 Level: {\\b ${esc(r.level)}} | Date: {\\b ${esc((r.submitted_at||"").slice(0,10))}} | Status: {\\b ${r.status==="corrected"?"Corrected":"Pending"}}\\par}\\par\n`;
    b += `{\\pard\\b\\fs22 Subject:\\b0\\par}{\\pard\\fs20\\i ${esc(PRETEST_SUBJECT.prompt)}\\i0\\par}\\par\n`;
    b += `{\\pard\\b\\fs22 Student Answer:\\b0\\par}{\\pard\\fs20 ${esc(r.writing_text)}\\par}\\par\n`;
    if(r.correction) b += `{\\pard\\b\\cf2\\fs22 Teacher Correction:\\b0\\cf0\\par}{\\pard\\fs20 ${esc(r.correction)}\\par}\\par\n`;
...
function WritingPretest({user,tok,level,onDone}){
  ...
  const words=wc(text);
  const enough=words>=PRETEST_SUBJECT.minWords;

  const submit=async()=>{
    if(!enough||!user?.id) return;
    setLoading(true);
    try{
      await post("writing_pretests",{
        user_id:user.id,
    ...
    <div style={{minHeight:"100vh",background:"#f0f7f4",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Segoe UI',sans-serif"}}>
      <div style={{width:"100%",maxWidth:440,textAlign:"center"}}>
        <Card>
          ...
          <h2 style={{color:"#2D6A4F",margin:"0 0 8px"}}>Submission Received!</h2>
          <p style={{color:"#555",fontSize:14,lineHeight:1.8}}>Your writing has been sent for teacher review. You will receive feedback soon.</p>
          ...
          <PBtn onClick={onDone} style={{background:"#2D6A4F"}}>Start Learning →</PBtn>
        </Card>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#f0f7f4",display:"flex",flexDirection:"column",alignItems:"center",padding:20,fontFamily:"'Segoe UI',sans-serif"}}>
      <div style={{width:"100%",maxWidth:440,paddingTop:16}}>
        <div style={{textAlign:"center",marginBottom:20}}>
          ...
          <h2 style={{color:"#1b4332",margin:"6px 0 2px"}}>Writing Pretest</h2>
          <p style={{color:"#888",fontSize:13}}>Your answer will be reviewed by a teacher</p>
        </div>
        <Card style={{background:"#e8f5e9",borderLeft:"4px solid #2D6A4F",marginBottom:14}}>
          <div style={{fontSize:12,color:"#555",fontWeight:700,marginBottom:6}}>📝 Subject</div>
          <p style={{fontWeight:600,color:"#1b4332",fontSize:15,lineHeight:1.7,margin:"0 0 8px"}}>{PRETEST_SUBJECT.prompt}</p>
          <p style={{fontSize:12,color:"#666",margin:0,lineHeight:1.6,fontStyle:"italic"}}>{PRETEST_SUBJECT.instructions}</p>
        </Card>
        <Card style={{marginBottom:14}}>
          <div style={{fontSize:12,color:"#888",marginBottom:8}}>Your answer (min. {PRETEST_SUBJECT.minWords} words)</div>
          <textarea
            value={text}
            onChange={e=>setText(e.target.value)}
            placeholder="Write your paragraph here…"
            rows={8}
            style={{width:"100%",boxSizing:"border-box",border:`2px solid ${enough?"#2D6A4F":words>0?"#f57c00":"#ddd"}`,borderRadius:12,padding:12,fontSize:14,resize:"vertical",outline:"none",fontFamily:"inherit",transition:"border .2s"}}
          />
          <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginTop:6}}>
            <span style={{color:enough?"#2D6A4F":words>0?"#f57c00":"#aaa",fontWeight:600}}>
              {words} / {PRETEST_SUBJECT.minWords} words {enough?"✅":words>0?"⚠️ Keep writing…":""}
            </span>
            {enough&&<span style={{color:"#2D6A4F"}}>Ready to submit!</span>}
          </div>
        </Card>
        {loading?<Spinner/>:<PBtn onClick={submit} disabled={!enough} style={{background:enough?"#2D6A4F":"#ccc"}}>Submit Writing ✍️</PBtn>}
      </div>
    </div>
  );
}
...
function AdminPanel({tok,G,LT,DK,onClose}){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const d=await get("writing_pretests?select=*&order=submitted_at.desc",tok);
  ...
  const saveCorrection=async()=>{
    ...
    setSaving(true);
    try{
      await patch(`writing_pretests?id=eq.${selected.id}`,{correction,status:"corrected"},tok);
      setSubmissions(s=>s.map(x=>x.id===selected.id?{...x,correction,status:"corrected"}:x));
      setSelected(s=>({...s,correction,status:"corrected"}));
    }catch(e){console.error(e);}
    setSaving(false);
  };
  ...
  if(selected) return(
    <div style={{padding:18}}>
      <button onClick={()=>{setSelected(null);setCorrection("");}} style={{background:"none",border:"none",color:G,fontWeight:700,fontSize:15,cursor:"pointer",padding:0,marginBottom:16}}>← Back to list</button>
      <Card style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
          <div>
            <div style={{fontWeight:800,color:DK,fontSize:16}}>{selected.student_code}</div>
            <div style={{fontSize:12,color:"#888"}}>Level: {selected.level} · {selected.submitted_at?.slice(0,10)}</div>
          </div>
          <span style={{background:selected.status==="corrected"?"#e8f5e9":"#fff3e0",color:selected.status==="corrected"?G:"#e65100",borderRadius:8,padding:"3px 12px",fontSize:12,fontWeight:700}}>
            {selected.status==="corrected"?"✅ Corrected":"⏳ Pending"}
          </span>
        </div>
      </Card>
      <Card style={{background:"#f9fbe7",marginBottom:12}}>
        <div style={{fontSize:12,color:"#888",fontWeight:700,marginBottom:6}}>📝 Subject</div>
        <p style={{fontSize:13,color:"#444",margin:0,fontStyle:"italic",lineHeight:1.7}}>{PRETEST_SUBJECT.prompt}</p>
      </Card>
      <Card style={{marginBottom:12}}>
        <div style={{fontSize:12,color:"#888",fontWeight:700,marginBottom:6}}>🧑‍🎓 Student Answer</div>
        <p style={{fontSize:14,color:"#333",margin:0,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{selected.writing_text}</p>
        <div style={{fontSize:12,color:"#aaa",marginTop:8}}>{wc(selected.writing_text)} words</div>
      </Card>
      <Card style={{marginBottom:12}}>
        <div style={{fontSize:12,color:G,fontWeight:700,marginBottom:8}}>✏️ Teacher Correction</div>
        <textarea
          value={correction||selected.correction||""}
          onChange={e=>setCorrection(e.target.value)}
          placeholder="Write your correction and feedback here…"
          rows={6}
          style={{width:"100%",boxSizing:"border-box",border:`2px solid ${G}`,borderRadius:12,padding:12,fontSize:14,resize:"vertical",outline:"none",fontFamily:"inherit"}}
        />
      </Card>
      <div style={{display:"flex",gap:10}}>
        {saving?<Spinner/>:<PBtn onClick={saveCorrection} style={{background:G,flex:1}}>💾 Save Correction</PBtn>}
        <button onClick={()=>exportWord([{...selected,correction:correction||selected.correction}],`pretest_${selected.student_code}.doc`)}
          style={{flex:1,background:"#1565c0",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",marginTop:8}}>
          📄 Export Word
        </button>
      </div>
    </div>
  ...
  return(
    <div style={{padding:18}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <h3 style={{color:DK,margin:0}}>🗂 Writing Pretests</h3>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#888",fontSize:20,cursor:"pointer"}}>✕</button>
      </div>
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        {["all","pending","corrected"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} style={{flex:1,background:filter===f?G:"#fff",color:filter===f?"#fff":DK,border:`1.5px solid ${filter===f?G:"#ddd"}`,borderRadius:20,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            {f==="all"?"All":f==="pending"?"⏳ Pending":"✅ Corrected"}
          </button>
        ))}
      </div>
      {filtered.length>0&&(
        <button onClick={()=>exportWord(filtered,`pretest_all_${dateStr()}.doc`)}
          style={{width:"100%",background:"#1565c0",color:"#fff",border:"none",borderRadius:12,padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit",marginBottom:14}}>
          📄 Export All ({filtered.length}) → Word
        </button>
      )}
      {loading&&<Spinner/>}
      {!loading&&filtered.length===0&&<Card style={{textAlign:"center",padding:32}}><div style={{fontSize:40,marginBottom:8}}>📭</div><p style={{color:"#888"}}>No submissions yet.</p></Card>}
      {filtered.map(s=>(
        <div key={s.id} onClick={()=>{setSelected(s);setCorrection(s.correction||"");}}
          style={{background:"#fff",border:`1.5px solid ${s.status==="corrected"?"#a5d6a7":"#ffe082"}`,borderRadius:14,padding:"14px 16px",marginBottom:10,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontWeight:800,color:DK,fontSize:14}}>{s.student_code}</div>
              <div style={{fontSize:12,color:"#888",marginTop:2}}>Level: {s.level} · {s.submitted_at?.slice(0,10)}</div>
              <div style={{fontSize:12,color:"#aaa",marginTop:2}}>{wc(s.writing_text)} words</div>
            </div>
            <span style={{background:s.status==="corrected"?"#e8f5e9":"#fff3e0",color:s.status==="corrected"?G:"#e65100",borderRadius:8,padding:"3px 10px",fontSize:11,fontWeight:700,flexShrink:0}}>
              {s.status==="corrected"?"✅":"⏳"}
            </span>
          </div>
          <p style={{fontSize:13,color:"#666",margin:"8px 0 0",lineHeight:1.5,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{s.writing_text}</p>
        </div>
      ))}
    </div>
  );
  ...
  const submit=async()=>{
    ...
    try{
      if(mode==="register"){
        ...
        const code=genCode();
        if(uid){
          await post("users",{id:uid,name:f.name,email:f.email,xp:0,streak:1,level:"Beginner",placement_done:false,last_login:dateStr(),student_code:code,anonymous_leaderboard:false},res.access_token);
          onDone({id:uid,name:f.name,email:f.email,xp:0,streak:1,level:"Beginner",isNew:true,token:res.access_token,student_code:code,anonymous_leaderboard:false});
        }
      }else{
...
function GrammarMod({addXp,onBack,G,LT,DK}){
  const [c]=useState(()=>rnd(GRAMMAR_BANK));const [sel,sSel]=useState(null);const [done,sDone]=useState(false);
...
function VocabMod({addXp,onBack,G,LT,DK}){
  const [c]=useState(()=>rnd(VOCAB_BANK));const [phase,sPhase]=useState("learn");const [sel,sSel]=useState(null);const [done,sDone]=useState(false);
  const conf=sel!==null,correct=sel===c.ans;
...
function ReadingMod({addXp,onBack,G,LT,DK}){
  const [c]=useState(()=>rnd(READING_BANK));const [phase,sP]=useState("read");const [ans,sA]=useState([null,null,null]);const [checked,sC]=useState(false);const [done,sD]=useState(false);
...
function MistakesMod({addXp,onBack,G,LT,DK}){
  const [c]=useState(()=>rnd(MISTAKES_BANK));const [done,sD]=useState(false);
...
function QuizMod({addXp,onBack,G,LT,DK}){
  const [qs]=useState(()=>rnd(QUIZ_SETS));const [i,sI]=useState(0);const [sel,sSel]=useState(null);const [score,sScore]=useState(0);const [review,sReview]=useState(false);const [done,sDone]=useState(false);
...
function PeelMod({addXp,onBack,level,G,LT,DK}){
  const [c]=useState(()=>rnd(PEEL_TOPICS));const [vals,sVals]=useState({point:"",explanation:"",evidence:"",link:""});const [step,sStep]=useState(0);const [phase,sPhase]=useState("write");const [done,sDone]=useState(false);
  ...
  if(done) return<DoneScreen xp={XP_MAP.peel} onBack={onBack} G={G} earn={()=>addXp(XP_MAP.peel,"peel")}/>;
  return(<div><Card style={{background:LT,marginBottom:14}}><div style={{fontSize:11,color:"#555"}}>📝 Topic</div><div style={{fontWeight:800,color:DK,fontSize:15,marginTop:2}}>{c.title}</div><div style={{color:"#555",fontSize:13,marginTop:4,lineHeight:1.6}}>{c.prompt}</div></Card><div style={{display:"flex",gap:6,marginBottom:16}}>{keys.map((k,ix)=>(<div key={k} style={{flex:1,textAlign:"center"}}><div style={{height:6,borderRadius:99,background:vals[k]&&wc(vals[k])>=minW[k]?G:vals[k]?"#f57c00":ix===step?"#81c784":"#e0e0e0",marginBottom:4}}/><div style={{fontSize:10,color:ix<=step?G:"#bbb",fontWeight:ix===step?800:400}}>{k.charAt(0).toUpperCase()}</div></div>))}</div><Card style={{marginBottom:10}}><div style={{fontWeight:700,color:DK,fontSize:14,marginBottom:6}}>{labels[step]}</div><div style={{fontSize:12,color:"#888",marginBottom:8}}>Min {minW[keys[step]]} words</div><textarea value={vals[keys[step]]} onChange={e=>sVals(p=>({...p,[keys[step]]:e.target.value}))} rows={4} style={{width:"100%",boxSizing:"border-box",border:`2px solid ${vals[keys[step]]&&wc(vals[keys[step]])>=minW[keys[step]]?G:vals[keys[step]]?"#f57c00":"#ddd"}`,borderRadius:12,padding:12,fontSize:14,resize:"vertical",outline:"none",fontFamily:"inherit"}}/><div style={{fontSize:12,color:wc(vals[keys[step]])>=minW[keys[step]]?G:wc(vals[keys[step]])>0?"#f57c00":"#aaa",marginTop:4,fontWeight:600}}>{wc(vals[keys[step]])} / {minW[keys[step]]} words {wc(vals[keys[step]])>=minW[keys[step]]?"✅":""}</div></Card><PBtn onClick={()=>{if(step<3)sStep(s=>s+1);else sDone(true);}} disabled={!vals[keys[step]]||wc(vals[keys[step]])<minW[keys[step]]} style={{background:G}}>{step<3?`Next: ${labels[step+1]} →`:"Complete PEEL ✍️"}</PBtn></div>);
}
...
function ProfileScreen({user,xp,lvl,level,badges,streak,anonymousLB,onToggleAnon,G,LT,DK}){
  return(<div style={{padding:18}}><div style={{background:`linear-gradient(135deg,${DK},${G})`,borderRadius:20,padding:24,color:"#fff",textAlign:"center",marginBottom:18}}><div style={{fontSize:52,marginBottom:8}}>👤</div><div style={{fontWeight:900,fontSize:20}}>{user?.name}</div><div style={{opacity:.75,fontSize:13,marginBottom:4}}>{user?.email}</div><div style={{background:"rgba(255,255,255,0.2)",borderRadius:10,padding:"4px 16px",display:"inline-block",marginBottom:8}}><span style={{fontWeight:800,fontSize:14}}>🎓 {user?.student_code}</span></div><div style={{display:"flex",justifyContent:"center",gap:28,marginTop:8}}>{[["⭐",xp,"XP"],["🔥",streak,"Streak"],["🏅",lvl.name,"Level"]].map(([ic,v,lb])=>(<div key={lb}><div style={{fontWeight:800,fontSize:17}}>{v}</div><div style={{fontSize:11,opacity:.75}}>{lb}</div></div>))}</div></div>
  <Card style={{marginBottom:18}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontWeight:700,color:DK,fontSize:14}}>👁 Leaderboard Display</div><div style={{fontSize:12,color:"#888",marginTop:2}}>{anonymousLB?"Showing your code":"Showing your name"}</div></div><button onClick={onToggleAnon} style={{background:anonymousLB?G:"#e0e0e0",color:anonymousLB?"#fff":"#555",border:"none",borderRadius:20,padding:"8px 16px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{anonymousLB?"🔒 Code":"👤 Name"}</button></div></Card>
  <h3 style={{color:DK,marginBottom:12}}>🏅 Badges</h3>{["Beginner","Intermediate","Advanced"].map(lv=>{const locked=["Beginner","Intermediate","Advanced"].indexOf(lv)>["Beginner","Intermediate","Advanced"].indexOf(level);const bdgs=BADGES_DEF[lv];return(<div key={lv} style={{marginBottom:20}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,padding:"8px 14px",borderRadius:12,background:locked?"#f5f5f5":LT,border:`1.5px solid ${locked?"#e0e0e0":G}`}}><span style={{fontWeight:800,color:locked?"#bbb":G,fontSize:15}}>{lv}</span>{locked&&<span style={{fontSize:12,color:"#bbb",marginLeft:"auto"}}>🔒 Locked</span>}{!locked&&<span style={{fontSize:12,color:G,marginLeft:"auto",fontWeight:600}}>{bdgs.filter(b=>badges.includes(b.name)).length}/{bdgs.length} earned</span>}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,opacity:locked?.4:1}}>{bdgs.map(b=>{const earned=badges.includes(b.name);return<div key={b.name} style={{background:earned?"#fff":"#f5f5f5",borderRadius:14,padding:14,boxShadow:earned?"0 2px 8px rgba(0,0,0,0.08)":"none",border:earned?`1.5px solid ${G}`:"none"}}><div style={{fontSize:28}}>{b.icon}</div><div style={{fontWeight:700,fontSize:13,color:DK,marginTop:4}}>{b.name}</div><div style={{fontSize:11,color:"#777",lineHeight:1.4}}>{b.desc}</div>{earned&&<div style={{fontSize:10,color:G,marginTop:4,fontWeight:700}}>✅ Earned</div>}</div>;})} </div></div>);})} </div>);
}

/* ═══════════════ LEADERBOARD ════════════════════════════ */
function BoardScreen({userId,myXp,tok,user,anonymousLB,G,LT,DK}){
...
function SettingsScreen({user,onTheme,onLogout,onAdmin,G,LT,DK}){
  ...
  <button onClick={onLogout} style={{width:"100%",marginTop:4,background:"#ffebee",color:"#c62828",border:"1.5px solid #ffcdd2",borderRadius:12,padding:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Log Out</button></div>);
}
...
export default function App(){
  ...
  const loadDone=async(uid,tk)=>{try{const d=await get(`daily_progress?user_id=eq.${uid}&date=eq.${dateStr()}&completed=eq.true&select=module`,tk);sDone(Array.isArray(d)?d.map(x=>x.module):[]);}catch{}};

  const afterAuth=async u=>{
    sUser(u);sTok(u.token);sXp(u.xp||0);sStreak(u.streak||1);
    setAnonymousLB(u.anonymous_leaderboard||false);
  ...
  const afterWritingPretest=async()=>{
    await loadDone(user?.id,tok);
    sScreen("app");
  };
  ...
  const toggleAnon=async()=>{
    const nv=!anonymousLB;
    setAnonymousLB(nv);
    if(user?.id) await patch(`users?id=eq.${user.id}`,{anonymous_leaderboard:nv},tok);
    sUser(u=>({...u,anonymous_leaderboard:nv}));
  };
  ...
  if(showAdmin) return(
    <div style={{maxWidth:440,margin:"0 auto",minHeight:"100vh",background:"#f0f7f4",fontFamily:"'Segoe UI',sans-serif"}}>
      <div style={{background:"#1565c0",color:"#fff",padding:"12px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{fontWeight:900,fontSize:16}}>🗂 Admin Panel</div>
        <button onClick={()=>setShowAdmin(false)} style={{background:"rgba(255,255,255,0.2)",border:"none",color:"#fff",borderRadius:8,padding:"6px 14px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>← Exit</button>
      </div>
      <AdminPanel tok={tok} G={G} LT={LT} DK={DK} onClose={()=>setShowAdmin(false)}/>
    </div>
  );
  ...
  return(
    <div style={{maxWidth:440,margin:"0 auto",minHeight:"100vh",background:"#f0f7f4",fontFamily:"'Segoe UI',sans-serif",display:"flex",flexDirection:"column"}}>
      <div style={{background:G,color:"#fff",padding:"12px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:10}}>
        <div><div style={{fontWeight:900,fontSize:16}}>✍️ WriteUP UPGC</div><div style={{fontSize:11,opacity:.75}}>{user?.name} · {level}</div></div>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:70}}>
        {mod
          ...
          :tab==="profile" ?<ProfileScreen user={user} xp={xp} lvl={lvl} level={level} badges={badges} streak={streak} anonymousLB={anonymousLB} onToggleAnon={toggleAnon} G={G} LT={LT} DK={DK}/>
          :tab==="board"   ?<BoardScreen userId={user?.id} myXp={xp} tok={tok} user={user} anonymousLB={anonymousLB} G={G} LT={LT} DK={DK}/>
